...
var Point, Shape, Segment, Label, Circle, Sphere, Box;
var init_objects = __esm({
  "src/projection-and-perspective/sketches/objects.ts"() {
    ...
    Shape = class _Shape {
      constructor(vertices, strokeColor, fillColor) {
        this.vertices = vertices;
        this.strokeColor = strokeColor;
        this.fillColor = fillColor;
      }
      render(p) {
        _Shape.renderShape(p, this.vertices, this.strokeColor, this.fillColor);
      }
      static renderShape(p, vertices, strokeColor, fillColor) {
        p.push();
    ...
    Segment = class extends Shape {
      constructor(start, end, color = "#000", N = 1) {
        const vertices = [];
        for (let i = 0; i <= N + 1; i++) {
          const t = i / (N + 1);
          vertices.push(add(scale(start, 1 - t), scale(end, t)));
        }
        super(vertices, color, void 0);
        this.start = start;
        this.end = end;
        this.strokeColor = color;
      }
      static renderSegment(p, start, end, color = "#000") {
        Shape.renderShape(p, [start, end], color, void 0);
      }
    };
    Label = class _Label {
      static {
      ...
      render(p) {
        _Label.renderLabel(p, this.position, this.text, this.color, this.up, this.right);
      }
      static renderLabel(p, position, text, color = "#000", up_, right_) {
        if (!_Label.font) {
          return;
        }
    ...
    Circle = class _Circle extends Shape {
      ...
      static renderCircle(p, center, radius, axis, color) {
        const circle = new _Circle(center, radius, axis, color);
        circle.render(p);
      }
    };
    Sphere = class _Sphere {
      ...
      render(p) {
        _Sphere.render(p, this.center, this.radius, this.color);
      }
      static render(p, center, radius, color = "#000") {
        p.push();
        p.translate(...center);
        setStyle(p, void 0, color);
...
// src/projection-and-perspective/sketches/accurate-circle.sketch.ts
var accurate_circle_sketch_exports = {};
__export(accurate_circle_sketch_exports, {
  create: () => create8
});
function create8(el, options) {
  return createP5Sketch(el, (p) => {
    ...
    p.draw = () => {
      ...
      const edgecenter = [P, Q, R, S];
      for (let i = 0; i < 4; i++) {
        Segment.renderSegment(p, corners[i], corners[(i + 1) % 4], "#000");
...
var init_accurate_circle_sketch = __esm({
  "src/projection-and-perspective/sketches/accurate-circle.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
// src/projection-and-perspective/sketches/circle-and-cube.sketch.ts
var circle_and_cube_sketch_exports = {};
__export(circle_and_cube_sketch_exports, {
  create: () => create9
});
function create9(el, options) {
  return createP5Sketch(el, (p) => {
    p.setup = () => {
      p.createCanvas(640, 480, p.WEBGL);
      p.camera(300, 200, 100, 0, 0, 0, 0, 0, -1);
    };
    function rot3(vec, i) {
      return [vec[i % 3], vec[(i + 1) % 3], vec[(i + 2) % 3]];
    }
    p.draw = () => {
...
var init_circle_and_cube_sketch = __esm({
  "src/projection-and-perspective/sketches/circle-and-cube.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
// src/projection-and-perspective/sketches/circle-basic.sketch.ts
var circle_basic_sketch_exports = {};
__export(circle_basic_sketch_exports, {
  create: () => create10
});
function create10(el, options) {
  return createP5Sketch(el, (p) => {
    p.setup = () => {
      p.createCanvas(640, 480, p.WEBGL);
      p.camera(-300, 300, -300, 0, 0, 0, 0, -1, 0);
...
var init_circle_basic_sketch = __esm({
  "src/projection-and-perspective/sketches/circle-basic.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function sequenceNumber(arr, t, stop, transition, mode = "cos") {
  ...
  if (t2 < stop) {
    ...
    const interp = mode === "linear" ? ratio : mode === "cos" ? (1 - Math.cos(ratio * Math.PI)) / 2 : mode === "cubic" ? 3 * ratio ** 2 - 2 * ratio ** 3 : ratio;
...
function create11(el, options) {
  return createP5Sketch(el, (p) => {
    ...
    p.draw = () => {
      ...
      p.background(255);
      const angle = sequenceNumber([0, 1], p.millis(), 1e3, 2e3);
      p.rotateX(angle);
      const radius = 80;
      const length = 400;
      const N = 32;
...
var init_circle_parallel_square_sketch = __esm({
  "src/projection-and-perspective/sketches/circle-parallel-square.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function create12(el, options) {
  return createP5Sketch(el, (p) => {
    ...
    p.draw = () => {
      ...
      const r = 200;
      const y = -40;
      const z = sequenceNumber([a, 0, -a, 0], p.millis(), 4e3, 1e3);
      const lines = [];
      const virtualLines = [];
      const N = 16;
      for (let i = 0; i < N; i++) {
...
var init_concurrent_sketch = __esm({
  "src/projection-and-perspective/sketches/concurrent.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function create13(el) {
  return createP5Sketch(el, (p) => {
    ...
    p.setup = () => {
      ...
      p.perspective(Math.PI / 2, p.width / p.height, 0.1, 1e3);
    };
    const N = 2;
    p.draw = () => {
      const t = p.millis() / 1e3;
...
var init_eccentric_circle_sketch = __esm({
  "src/projection-and-perspective/sketches/eccentric-circle.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function create18(el) {
  return createP5Sketch(el, (p) => {
    ...
    p.draw = () => {
      ...
      const t = p.millis() / 4e3 * Math.PI * 2;
      const point = add(circle1.center, scale([Math.cos(t), 0, Math.sin(t)], r));
...
function create19(el, options) {
  ...
  return createP5Sketch(el, (p) => {
    ...
    const vec = [-1.2, 1, 3];
    const pos = cross(vec, [-5, -5, -5]);
    const eye = [0, 0, 0];
    const A = sub(pos, scale(vec, 100));
    const B = add(pos, scale(vec, 100));
    const C = sub(scale(pos, 10), scale(vec, 100));
    const D = add(scale(pos, 10), scale(vec, 100));
    const E = sub(eye, scale(vec, 100));
    const F = add(eye, scale(vec, 100));
    p.setup = () => {
      p.createCanvas(640, 480, p.WEBGL);
      camera = new IdealCamera(eye, [0, 0, 100], 200, 150);
      line = new Segment(A, B, "#000");
      p.camera(-300, 300, -200, 0, 0, 100, 0, -1, 0);
    };
    p.draw = () => {
      p.background(255);
      p.orbitControl();
      const x = Math.abs(p.millis() / 3e3 % 2 - 1);
      let t = Math.tan((x - 0.5) * Math.PI) * mag(sub(eye, pos));
      if (!isFinite(t)) t = 100 * Math.sign(x);
      const point = add(pos, scale(vec, t));
      ...
      line.render(p);
      camera.renderTrace(p, point, 0, "#000", "#f00", options.virtual ? "#00f" : void 0, {
        renderImageLocus: true
      });
      camera.outerRender(p, { critical: true });
      Shape.renderShape(p, [C, D, F, E], void 0, "#f001");
      camera.canvasRender(p);
    };
  });
}
var init_image_of_line_sketch = __esm({
  "src/projection-and-perspective/sketches/image-of-line.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function create21(el, options) {
  ...
  return createP5Sketch(el, (p) => {
    ...
    p.draw = () => {
      ...
      p.orbitControl();
      const x = Math.abs(p.millis() / 3e3 % 2 - 1);
      const point = add(scale(A, 1 - x), scale(B, x));
      const image = camera.solve(point);
      const a = camera.solve(A);
      const b = camera.solve(B);
      const Aex = add(A, scale(sub(A, camera.eye), 1));
      const Bex = add(B, scale(sub(B, camera.eye), 1));
      ...
      Shape.renderShape(p, [Aex, Bex, camera.eye], void 0, "#f001");
      camera.canvasRender(p);
    };
  });
}
var init_image_of_segment_sketch = __esm({
  "src/projection-and-perspective/sketches/image-of-segment.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function create23(el, options) {
  return createP5Sketch(el, (p) => {
    ...
    p.setup = () => {
      p.createCanvas(640, 480, p.WEBGL);
      p.camera(-300, -300, -300, 0, 0, 0, 0, 0, 1);
    };
    p.draw = () => {
      const t = p.millis() / 1e3;
      p.background(255);
...
var init_orthodrome_sketch = __esm({
  "src/projection-and-perspective/sketches/orthodrome.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
function rotateVector(v, angle, axis) {
  ...
  return [
    cos * v[0] + sin * (axis[1] * v[2] - axis[2] * v[1]) + (1 - cos) * dot2 * axis[0],
    cos * v[1] + sin * (axis[2] * v[0] - axis[0] * v[2]) + (1 - cos) * dot2 * axis[1],
    cos * v[2] + sin * (axis[0] * v[1] - axis[1] * v[0]) + (1 - cos) * dot2 * axis[2]
  ];
}
function planeIntersect(normal, z, color) {
  normal = normalize(normal);
  const d = Math.sqrt(normal[0] * normal[0] + normal[1] * normal[1]);
  if (d === 0) return new Segment([0, 0, z], [0, 0, z], color);
  const vec = cross(normal, [0, 0, 1]);
  let pos = cross(normal, vec);
  pos = scale(pos, z / pos[2]);
  return new Segment(add(pos, scale(vec, -1e3)), add(pos, scale(vec, 1e3)), color);
}
function renderHemicircle(p, normal, r, upperColor, lowerColor) {
  normal = normalize(normal);
  const N = 32;
  const axis1 = normalize(cross(normal, [0, 0, 1]));
  ...
  return createP5Sketch(el, (p) => {
    ...
    p.draw = () => {
      ...
      for (let i = 0; i < lineNum; i++) {
        ...
        const tiltAngle = p.noise(seed[i] + 1e3, t / 100) * Math.PI * 2 * 10;
...
var init_projective_geometry_sketch = __esm({
  "src/projection-and-perspective/sketches/projective-geometry.sketch.ts"() {
    "use strict";
    init_sketch_helper();
    init_objects();
...
var sketchManifest = {
  ...
  "projection-and-perspective/sketches/image-of-line": { loader: () => Promise.resolve().then(() => (init_image_of_line_sketch(), image_of_line_sketch_exports)) },
  ...
  "projection-and-perspective/sketches/image-of-segment": { loader: () => Promise.resolve().then(() => (init_image_of_segment_sketch(), image_of_segment_sketch_exports)) },
  ...
  "projection-and-perspective/sketches/orthodrome": { loader: () => Promise.resolve().then(() => (init_orthodrome_sketch(), orthodrome_sketch_exports)) },
  "projection-and-perspective/sketches/projective-geometry": { loader: () => Promise.resolve().then(() => (init_projective_geometry_sketch(), projective_geometry_sketch_exports)) },
